...
static void __setClientFactory() __attribute__((constructor));

static void __setClientFactory() {
	// check that there is no other factory already defined. First defined First used
	MqttClient::setFactory([]() -> std::unique_ptr<MqttClientInterface> {
		return std::unique_ptr<MqttClientInterface>(
			new ZephyrMqttClient()
		);
	});
}
...
void ZephyrMqttClient::disconnect() { // TODO call this in the destructor
	mqtt_disconnect(&client, nullptr); // TODO mqtt_disconnect_param takes values in case of mwtt version 5
...
error_t ZephyrMqttClient::subscribe(Topic t, MqttQos qos) {
	...
	struct mqtt_topic topics[] = {{
		...
		.qos = convertQosValue(qos), // FIXME
	}};

	const struct mqtt_subscription_list sub_list {
		.list = topics,
		.list_count = 1,
		.message_id = sys_rand16_get() // 5841uv // TODO what value should this take?
	};

	return mqtt_subscribe(&client, &sub_list); // should this parameter kept in an available storage
}

error_t ZephyrMqttClient::publish(Topic t, uint8_t payload[], size_t size, MqttQos qos) {
	...
	param.dup_flag = 0U; 	// TODO this should be a parameter
	param.retain_flag = 0U; // TODO this should be a parameter
...
void ZephyrMqttClient::poll() {
	...
	// Serial.print("_mqtt_keepalive_time_left ");
	// Serial.println(_mqtt_keepalive_time_left);
...
class ZephyrMqttReadStream: public Stream {
public:
	ZephyrMqttReadStream(struct mqtt_client *client, const struct mqtt_publish_param *p)
	: client(client), p(p), ended(false) {}

    ~ZephyrMqttReadStream() { end(); }

    size_t readBytes(char* buf, size_t s) override {
    ...
    int peek() override { return -1; } // I don't think this could be implemented

	size_t write(uint8_t) override { /* not needed */ return 0; };
...
private:
	...
	bool ended; // FIXME there is a better way
};
...
void ZephyrMqttClient::setClientId(const char* client_id) {
	client.client_id.utf8 = (const uint8_t*)client_id;
	client.client_id.size = strlen((const char*)client_id);
}

void ZephyrMqttClient::setAuth(const char* username, const char* password) {
...
void ZephyrMqttClient::setWill(Topic will_topic, const uint8_t* will_message, size_t will_size, MqttQos qos) {
